...
export default function Iptelefonlar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.comtech-ombori.uz/comtech?type=iptelefonlar');
        if (!response.ok) {
        ...
        if (data && Array.isArray(data.user)) {
          const Rows = data.user.filter(row => row.type === 'iptelefonlar');

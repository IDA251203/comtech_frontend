...
export default function Radiomostlar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://api.comtech-ombori.uz/comtech?type=radiomostlar');
        if (!response.ok) {
        ...
        if (data && Array.isArray(data.user)) {
          ...
          const Rows = data.user.filter(row => row.type === 'radiomostlar');
